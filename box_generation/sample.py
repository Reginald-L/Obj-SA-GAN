...
if opt.is_training and opt.load_checkpoint is None:
    ...
    if not opt.resume:
        ...
        decoder = DecoderRNN(train_label_lang.word2index, x_mean_std[0], y_mean_std[0], 
            ...
            opt.gmm_comp_num, dropout_p=0.2, use_attention=use_attention, bidirectional=bidirectional)
        print(decoder)
    ...
    train_cap_lang, train_label_lang, train_tuples, dev_cap_lang, dev_label_lang, dev_tuples, \
    x_mean_std, y_mean_std, w_mean_std, r_mean_std = prepare_data(opt.train_path,
                                                                  opt.dev_path, opt.mean_std_path, opt.max_len,
                                                                  opt.min_len, ixtoword, wordtoix)
    ...
    if not opt.resume:
        ...
        decoder = DecoderRNN(train_label_lang.word2index, x_mean_std[0], y_mean_std[0],
                             w_mean_std[0], r_mean_std[0], opt.batch_size, opt.max_len, hidden_size,
                             opt.gmm_comp_num, dropout_p=0.2, use_attention=use_attention, bidirectional=bidirectional)
        print("load pre decoder params")
        print(pre_decoder.state_dict().keys())
        print(decoder.state_dict().keys())
        decoder.load_state_dict(pre_decoder.state_dict(),strict=False)
        print(decoder)
    ...
    seq2seq = t.train(encoder, decoder, train_tuples, num_epochs=5, dev_data=dev_tuples,
                      optimizer=optimizer, resume=opt.resume, is_training=opt.is_training)
